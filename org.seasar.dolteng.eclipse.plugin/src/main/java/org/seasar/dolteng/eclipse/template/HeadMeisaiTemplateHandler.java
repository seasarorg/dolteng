 ...
 * 
 */
public class HeadMeisaiTemplateHandler extends AbstractTemplateHandler implements
        TemplateHandler {

    private ScaffoldHeadMeisaiConfig config;

    private int templateCount = 0;
    
    // selectedColumns にテーブル上の選択された列情報が格納されています。
    public HeadMeisaiTemplateHandler(ScaffoldHeadMeisaiConfig config, IProject project,
        ...
        super(project, monitor, config.getModelFactory().createScaffoldModel(createVariables(node
                .getMetaData().getName(), project, selectedColumns, meisaiTableName), node, selectedColumns,
                meisaiTableName, meisaiColumns));
        this.config = config;
    }

    @SuppressWarnings("unchecked")
    protected static Map<String, String> createVariables(String tableName, IProject project,
            Map<Integer, String[]> selectedColumns, String meisaiTableName) {
        ...
        {
            String table = NameConverter.toCamelCase(meisaiTableName);
            result.put("meisaitable", StringUtil.decapitalize(table));
            result.put("meisaitable_capitalize", table);
            result.put("meisaitable_rdb", meisaiTableName);
        }
